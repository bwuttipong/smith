...
For beautiful, readable output (especially important for users who prefer emojis and clean formatting):

1. Use `--json` flag and parse the output to extract meaningful information
2. Map priority numbers to symbols: 🔴 P1 (high), 🟠 P2, 🟡 P3, ⚪ P4 (low)
3. Clean up content by removing markdown links but keeping the text and emojis
4. Format due dates in a human-readable way (today, tomorrow, X days, or Month Day)
5. Show labels as @labelname
6. List tasks with clear numbering and visual priority indicators

Example clean format:
```
1. 🔴 Perform a workday shutdown routine 🌖 @read
2. 🟠 Review my day and plan ahead (ev workday at 5pm)
3. ⚪ Learn the basics with the easy-to-follow Getting Started Guide 🔗 (Jul 12)
```

This approach avoids showing confusing internal task IDs and focuses on what matters to the user.
