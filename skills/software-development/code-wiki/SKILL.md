...
- **ClawHub naming collision.** A separate skill called `codewiki` (no hyphen) exists on ClawHub — it depends on the external `FSoft-AI4Code/CodeWiki` CLI binary and will fail if that tool isn't installed. This skill (`code-wiki`, with hyphen) has zero external dependencies and should be the default. If a user says "install code wiki" and the CLI install is blocked, they're hitting the ClawHub slug mismatch (`codewiki` vs `code-wiki`) or the missing binary — just use this skill directly.
