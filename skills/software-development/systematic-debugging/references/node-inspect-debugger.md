...
kill -SIGUSR1 <pid>
# Node prints: Debugger listening on ws://***.***.*.*:9229/<uuid>

# 2. Attach the debugger CLI
node inspect -p <pid>
# or by URL
node inspect ws://***.***.*.*:9229/<uuid>
```
...
# 3. Find the WS URL
curl -s http://***.***.*.*:9229/json/list | jq -r '.[0].webSocketDebuggerUrl'
...
### Debugging `tui_gateway` / PTY child processes

Those are Python, not Node — use the `python-debugpy` reference for them. Only Node portions (Ink UI, tui_gateway client, tsx-run tests under `ui-tui/`) use this skill.
...
In another terminal: `node inspect -p <pid>`, then `sb('src/app.tsx', 42)`, `cont`.
...
3. **Port collisions.** Default is `9229`. If multiple Node processes are inspecting, pass `--inspect=0` (random port) and read the actual URL from `/json/list`:
   ```bash
   curl -s http://***.***.*.*:9229/json/list   # lists all inspectable targets on the host
   ```
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Hermes, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`.

7. **Security.** `--inspect=0.0.0.0:9229` exposes arbitrary code execution. Always bind to `***.***.*.*` (the default) unless you have an isolated network.

## Verification Checklist

- [ ] `curl -s http://***.***.*.*:9229/json/list` returns exactly the target you expect
